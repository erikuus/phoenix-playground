defmodule LivePlaygroundWeb.StepsLive.Filtered.FormComponent do
  ...
  def update(%{language: language} = assigns, socket) do
    changeset = FilteredLanguages.change_language(language)
  ...
  defp save_language(socket, :edit, language_params) do
    ...
    case FilteredLanguages.update_language(language, language_params) do
      {:ok, updated_language} ->
      ...
      {:error, %Ecto.Changeset{} = changeset} ->
        if changeset.errors[:lock_version] do
          ...
          socket =
            ...
            |> assign_form(FilteredLanguages.change_language(latest_language))
            |> put_flash(
              :lock,
  ...
  defp save_language(socket, :new, language_params) do
    case FilteredLanguages.create_language(language_params) do
      {:ok, language} ->
