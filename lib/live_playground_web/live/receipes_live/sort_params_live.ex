defmodule LivePlaygroundWeb.SortParamsLive do
  ...
  def render(assigns) do
    ...
    <.header class="mb-6">
      ...
      <:subtitle>
        How to handle sorting parameters in LiveView
      </:subtitle>
      <:actions>
        <.link navigate={~p"/sort"}>
          <.icon name="hero-arrow-long-left" class="mr-1 h-5 w-5 text-gray-400" /> Back to: Sorting
        </.link>
      </:actions>
    </.header>
    ...
    <div class="mt-10 space-y-6">
      <%= raw(code("lib/live_playground_web/live/receipes_live/sort_params_live.ex")) %> <%= raw(
